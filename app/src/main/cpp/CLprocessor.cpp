...
int initCL()
{
    ...
    try
    {
        ...
        cl::Program::Sources src(1, {oclProgI2I, sizeof(oclProgI2I)});
...
void drawFrameNoProc(int w, int h, int texOut)
{
    LOGD("No Processing");
    int64_t t;
...
void processFrame(int tex1, int tex2, int w, int h, int mode)
{
    ...
    case PROC_MODE_NO_PROC:
        LOGD("Processing mode (NO Proc): %d", mode);
        drawFrameNoProc(w, h, tex2);
        break;
    case PROC_MODE_CPU:
