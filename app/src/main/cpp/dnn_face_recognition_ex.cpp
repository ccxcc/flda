...
int fc_main(const char *pfile) {
    try {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "*********** start face recog test\n");

        // The first thing we are going to do is load all our models.  First, since we need to
        // find faces in the image we will need a face detector:
        frontal_face_detector detector = get_frontal_face_detector();
        // We will also use a face landmarking model to align faces to a standard pose:  (see face_landmark_detection_ex.cpp for an introduction)
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "*********** start face recog landmark model loading\n");
        shape_predictor sp;
        deserialize("/data/local/tmp/shape_predictor_68_face_landmarks.dat") >> sp;
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "*********** start face recog landmark model loaded\n");
        // And finally we load the DNN responsible for face recognition.
        anet_type net;
        deserialize("/data/local/tmp/dlib_face_recognition_resnet_model_v1.dat") >> net;

        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "*********** start face recog load image file\n");

        matrix<rgb_pixel> img;
        load_image(img, pfile);
...
//    image_window win(img);
        ...
        // and centered.
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "*********** start face recog face detect\n");
        std::vector<matrix<rgb_pixel>> faces;
        for (auto face : detector(img)) {
            __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "*********** start face recog face detected\n");
...
//        win.add_overlay(face);
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, " found faces: %d\n", faces.size());
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, " recog faces: %d\n", face_descriptors.data()->size());

        return face_descriptors.size();
